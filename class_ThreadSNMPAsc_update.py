#
import time
import os
import sys
import sqlite3
from pathlib import Path
from snmp import Manager
from snmp import exceptions
from snmp.exceptions import Timeout
from snmp.v1.exceptions import NoSuchName
# Импортируем классс QThread отвечающий за многопоточность в PYQT
from PyQt5.QtCore import QThread
from class_ClientModbus import ClientModbus
from class_SqlLiteMain import ConnectSqlDB
print 
class ThreadSNMPAsk(QThread, ClientModbus):
    def __init__(self):
        # Запускаем у класса QThread метод init
        QThread.__init__(self)
    
        # Переменная определяет время между запросами цикла while
        self.interval_time = 5
        #
        self.snmp_trap = []
        #
        self.snmp_trap1 = []
        # Переменная queue, которой присваиваем значение экземпляр класса queue ( обработчик очередей) полученной
        # при вызове данного класса
        #self.queue = queue
        ''' 
        self.queue.put(sys.exc_info())
        Вызываем у экземпляра класса queue метод put, который помещает элемен в очередь.
        Этот метод никогда не блокируется и всегда завершается успешно.
        В качестве элемента передаем метод exc_info модуль sys, который возвращает информацию 
        о самом последнем исключении, перехваченном в текущем фрейме стека или в более раннем фрейме стека.

        '''
        # Создание переменных OID
        self.load_A = '1.3.6.1.4.1.33183.14.3.2.0'
        self.temp = '1.3.6.1.4.1.33183.14.5.1.4.1'
        self.load_V = '1.3.6.1.4.1.33183.14.3.1.0'
        self.out_V = '1.3.6.1.4.1.33183.14.5.1.2.1'
        self.in_st = '1.3.6.1.4.1.33183.14.2.3.0'
        self.batt = '1.3.6.1.4.1.33183.14.5.1.6.1'
        self.inp_V = '1.3.6.1.4.1.33183.14.2.1.0'
        self.load_Amp ='1.3.6.1.4.1.33183.10.3.2.0'
        self.tmp = '1.3.6.1.4.1.33183.10.4.1.4.1'
        self.load_Vol = '1.3.6.1.4.1.33183.10.3.1.0'
        self.in_stat = '1.3.6.1.4.1.33183.10.2.3.0'
        self.betery = '1.3.6.1.4.1.33183.10.5.1.6.1'
        self.inp_Volt = '1.3.6.1.4.1.33183.10.2.1.0'
        # EATON
        self.power = '1.3.6.1.2.1.33.1.4.4.1.4.1'
        self.load = '1.3.6.1.2.1.33.1.4.4.1.5.1'
        self.out = '1.3.6.1.2.1.33.1.4.4.1.2.1'
        self.inp = '1.3.6.1.2.1.33.1.3.3.1.3.1'
        self.battery = '1.3.6.1.2.1.33.1.2.4.0'
        self.in_status = '1.3.6.1.2.1.33.1.4.1.0'
        self.temperature = '1.3.6.1.4.1.534.1.6.1.0'

        self.load_Am = '1.3.6.1.4.1.1918.2.13.10.50.10.0'
        self.out_Vol = '1.3.6.1.4.1.1918.2.13.10.70.10.20.0'
        self.inp_Vol = '1.3.6.1.4.1.1918.2.13.10.40.10.0'
        self.temper = '1.3.6.1.4.1.1918.2.13.10.100.30.0'
        self.tmprt = '1.3.6.1.2.1.33.1.2.7.0'
        # APC 
        self.output_load = '1.3.6.1.4.1.318.1.1.1.4.2.3.0'
        self.output_voltage = '1.3.6.1.4.1.318.1.1.1.4.2.1.0'
        self.inpu_Voltage = '1.3.6.1.4.1.318.1.1.1.3.2.1.0'
        self.input_st = '1.3.6.1.4.1.318.1.1.1.4.1.1.0'
        self.batt_temp = '1.3.6.1.4.1.318.1.1.1.2.2.2.0'
        self.batt_capacity = '1.3.6.1.4.1.318.1.1.1.2.2.1.0'
        # Eltek OIDs
        self.rect_stat_temp = '1.3.6.1.4.1.12148.9.5.5.2.1.5'
        self.batt_out_volt = '1.3.6.1.4.1.12148.9.3.2'
        self.rect_total_curr = '1.3.6.1.4.1.12148.9.5.3'
        self.ac_volt1 = '1.3.6.1.4.1.12148.9.6.1' 
        # MAC&C OIDs
        self.f10GFx2DDTemp = '1.3.6.1.4.1.40844.100.270.20.1.60'
        self.f10GFx2DDTxPWR = '1.3.6.1.4.1.40844.100.270.20.1.58'
        self.f10GFx2DDRxPWR = '1.3.6.1.4.1.40844.100.270.20.1.59'
        self.f10GFx3DDTemp = '1.3.6.1.4.1.40844.100.270.20.1.74'
        self.f10GFx3DDRxPWR = '1.3.6.1.4.1.40844.100.270.20.1.73'
        self.f10GFx3DDTxPWR = '1.3.6.1.4.1.40844.100.270.20.1.72'

    ''' 
    GetRequest - запрос к агенту от менеджера, используемый для получения значения одной или нескольких переменных.
    GetNextRequest - запрос к агенту от менеджера, используемый для получения следующего в иерархии значения переменной.
    '''

    def get_request(self, ip, oids, flag, timeout, next):
        if flag:
            self.manager = Manager(b'public', version=1, port=161)
        try:
            out = self.manager.get(ip, *oids, next=False, timeout=timeout)
            if flag:
                self.manager.close()
            return out
        except Timeout as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout - не получен ответ от устройства {ip}')
           # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName - не существующая переменная {err}: {ip}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError - SNMP Manager не запущен, в запросе не передан flag=True: {ip}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)

    def macc(self, ip, timeout=10, flag=None):
        if flag:
            self.manager = Manager(b'public', version=1, port=161)
        #
        values = []
        oids = [self.f10GFx2DDTemp, self.f10GFx2DDTxPWR, self.f10GFx2DDRxPWR, self.f10GFx3DDTemp, self.f10GFx3DDTxPWR, self.f10GFx3DDRxPWR]
        try:
            # Делаем запрос GETNEX получаем список данных типа snmp.types.VarBind
            out = self.manager.get(ip, *oids, next=True, refresh=True, timeout=timeout)
            # Перебираем список по переменной типа VarBind
            for var_bind in out:
                # Используем метод values, получаем из VarBind данные типа snmp.types.OCTET_STRING, значение в которой в байтах
                snmp_octet_string = var_bind.values
                # Используем метод value получаем из OCTET_STRING значение в байтах преобразуем его в строку и добавляем в список values
                values.append(snmp_octet_string[-1].value.decode())
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
           # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName - не существующая переменная {err}: MAC&C {ip}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} TxFiber2: {values[1]} dBm; RxFiber2: {values[2]} dBm; TempFiber2: {values[0]} *C; TxFiber3: {values[4]} dBm; RxFiber3: {values[5]} dBm; TempFiber3: {values[3]} *C" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def eltek(self, ip, flag = False, timeout=10):
        if flag:
            self.manager = Manager(b'public')
        values = []
        oids = [self.ac_volt1, self.batt_out_volt, self.rect_total_curr, self.rect_stat_temp]
        try:
            # установка параметра next в True отправит запрос GETNEXT, а не GET запрос
            out = self.manager.get(ip, *oids, next=True, refresh=True, timeout=timeout)
            for i in out:
                tuple_snmp_data = i.values
                if tuple_snmp_data[-2].value.rstrip('.0') == self.batt_out_volt:
                    item = tuple_snmp_data[-1].value / 100
                    values.append(item)
                else:
                    values.append(tuple_snmp_data[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            # Возвращаем результат (это нужно для Приложения, когда мы делаем проверку к какой группе относится устройство и что бы
            # узнать, что метод корректно отработал или нет, нам нужно получить данные)
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName, попробуйте другой вид запроса: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[0]} V; OUT: {values[1]} V ({values[2]} А); *C: {values[3]}"  
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        # Возвращаем результат (это нужно для Приложения, когда мы делаем проверку к какой группе относится устройство и что бы
        # узнать, что метод корректно отработал нам нужно получить данные)
        return result
        
    def forpost_2(self, ip, timeout=10, next = False, flag = False):
        #if flag:
            #self.manager = Manager(b'public', version=1)
        values = []
        oids = [self.load_A, self.temp, self.load_V, self.out_V, self.in_st, self.batt, self.inp_V]
        #try:
            #out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
        out = self.get_request(ip, oids, flag, timeout, next)
        if type(out) is list:
            for i in out:
                snm = i.values
                if snm[-2].value == self.out_V or snm[-2].value == self.load_A:
                    item = snm[-1].value / 10
                    values.append(item)
                else:
                    values.append(snm[-1].value)
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[6]} V [status: {values[4]}]; OUT: {values[3]} V ({values[0]} А). Batt: {values[5]}%. *C: {values[1]}"  
            # Если flag истина, то закрываем SNMP поток. 
            #if flag:
                #self.manager.close()
            return result
        else:
            return out
        #except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            #error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            #self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            #if flag:
                #self.manager.close()
            # Возвращаем результат (это нужно для Приложения, когда мы делаем проверку к какой группе относится устройство и что бы
            # узнать, что метод корректно отработал или нет, нам нужно получить данные)
            #return error
        #except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            #error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            #self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            #if flag:
                #self.manager.close()
            #return error
        #except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            #error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            #self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            #if flag:
                #self.manager.close()
            #return error
        #except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            #if flag:
                #self.manager.close()
            #return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        #result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[6]} V [status: {values[4]}]; OUT: {values[3]} V ({values[0]} А). Batt: {values[5]}%. *C: {values[1]}"  
        # Если flag истина, то закрываем SNMP поток. 
        #if flag:
            #self.manager.close()
        # Возвращаем результат (это нужно для Приложения, когда мы делаем проверку к какой группе относится устройство и что бы
        # узнать, что метод корректно отработал нам нужно получить данные)
        #return result

    def forpost(self, ip, timeout=10, flag = None):
        if flag:
            self.manager = Manager(b'public', version=1)
        # 
        values = []
        oids = [self.load_Amp, self.tmp, self.load_Vol, self.in_stat, self.betery, self.inp_Volt]
        try:
            # Делаем запрос, результат записываем в переменную out
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)

            for i in out:
                snm = i.values
                # Делаем проверку если полученное значение равно load_Vol или load_Amp
                if snm[-2].value == self.load_Vol or snm[-2].value == self.load_Amp:
                    # Полученное значение делим на 10, что бы получить корректные значения Напряжения и Тока нагрузки
                    item = snm[-1].value / 10
                    # Полученные значения добавляем в список values
                    values.append(item)
                else:
                    values.append(snm[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка: {str(err)} ')
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[5]} V [status: {values[3]}]; OUT: {values[2]} V ({values[0]} А); Batt: {values[4]}%; *C: {values[1]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def forpost_3(self, ip, timeout=10, flag = None):
        if flag:
            self.manager = Manager(b'public', version=1)
        values = []
        oids = [self.load_Amp, self.tmp, self.load_Vol, self.in_stat, self.inp_Volt]
        try:
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
            for i in out:
                snm = i.values
                if snm[-2].value == self.load_Vol or snm[-2].value == self.load_Amp:
                    item = snm[-1].value / 10
                    values.append(item)
                else:
                    values.append(snm[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[4]} V [status: {values[3]}]; OUT:{values[2]} V ({values[0]} А);*C: {values[1]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def eaton(self, ip, timeout=10, flag=None):
        if flag:
            self.manager = Manager(b'public', version=1)
        values = []
        oids = [self.power, self.load, self.out, self.inp, self.battery, self.in_status, self.temperature]
        try:
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
            for i in out:
                snm = i.values
                values.append(snm[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[3]} V [status: {values[5]}]; OUT:{values[2]} V; Load: {values[1]}% ({values[0]} WT); Batt: {values[4]}%; *C: {values[6]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def sc200(self, ip, timeout=10, flag=None):
        if flag:
            self.manager = Manager(b'public', version=1)
        values = []
        oids = [self.load_Am, self.out_Vol, self.inp_Vol, self.temper]
        try:
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
            for i in out:
                snm = i.values
                if snm[-2].value == self.out_Vol:
                    # Полученное значение делим на 100, что бы получить корректные значения Выходного напряжения
                    item = snm[-1].value / 100
                    # Полученные значения добавляем в список values
                    values.append(item)
                else:
                    values.append(snm[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[2]} V; OUT:{values[1]} _V Load: ({values[0]}A); *C: {values[3]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def megatec(self, ip, timeout=10, flag=None):
        if flag:
            self.manager = Manager(b'public', version=1)
        values = []
        oids = [self.out, self.inp, self.tmprt, self.battery]
        try:
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
            for i in out:
                snm = i.values
                values.append(snm[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except OSError as err:
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return str(err)
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[1]} V; OUT:{values[0]} V Batt: {values[3]}%; *C: {values[2]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    def apc(self, ip, timeout=10, flag=None):
        if flag:
            self.manager = Manager(b'public', version=1)
        #
        values = []
        oids = [self.output_load, self.output_voltage, self.inpu_Voltage, self.input_st, self.batt_temp, self.batt_capacity]
        try:
            # Делаем GET запрос, получаем список данных типа VarBind и записываем в перенменную out
            out = self.manager.get(ip, *oids, refresh=True, timeout=timeout)
            # Перебираем список с данными типа VarBind 
            for var_bind in out:
                # Используем метод values, получаем из VarBind данные типа SNMP_INTEGER
                snm_integer = var_bind.values
                # Используем метод value получаем из SNMP_INTEGER числовое значение и добавляем в список values
                values.append(snm_integer[-1].value)
        except Timeout:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            # Логирование, записывает в файл logs_snmp_ask.txt
            self.logger.info(f'Ошибка Timeout. Устройство {ip} не доступно')
           # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except NoSuchName as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка NoSuchName: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        except AttributeError as err:
            # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
            error = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} SNMP:REQEST_ERROR"
            self.logger.info(f'Ошибка AttributeError: {err}')
            # Если flag истина, то закрываем SNMP поток. 
            if flag:
                self.manager.close()
            return error
        # Подставляем дату(перепреобразовав в нужный формат) и ip адрес
        result = f"{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())} {ip} IN: {values[2]} V  [status: {values[3]}]; OUT: {values[1]} V; Load: {values[0]}%; Batt: {values[5]}%; *C: {values[4]}" 
        # Если flag истина, то закрываем SNMP поток. 
        if flag:
            self.manager.close()
        return result

    # Метод запускает SNMP опрощик
    def run(self):
        while True:
            with Manager(b'public', version=1) as self.manager:
                # Создаем переменную в которую будем сохранять результаты snmp
                self.snmp_trap = []
                # Создаем экземпляр класса
                #sql = ConnectSqlDB()
                with ConnectSqlDB() as sql:
                    try:
                        # Делаем запрос к БД, получить список значений oid и ip из таблицы Devices
                        data_base = sql.get_values_list_db('model', 'ip', table='Devices')
                    except (sqlite3.IntegrityError, sqlite3.OperationalError, sqlite3.NoneType):
                        self.logger.error("Ошибка запроса из БД: sql.get_values_list_db('key', 'ip', table='Devices')" )
                if data_base:
                    # Перебираем данные словаря по ключам
                    for key, ip in data_base:
                        # Делаем проверку если ключ словаря сопал с названием метода
                        if key == 'forpost':
                            # Вызываем метод forpost передавая ему ip адрес устройства
                            result = self.forpost(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'forpost_2':
                            # Вызываем метод forpost_2 передавая ему ip адрес устройства
                            result = self.forpost_2(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'forpost_3':
                            # Вызываем метод forpost_3 передавая ему ip адрес устройства
                            result = self.forpost_3(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'eaton':
                            # Вызываем метод eaton передавая ему ip адрес устройства
                            result = self.eaton(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'sc200':
                            # Вызываем метод sc200 передавая ему ip адрес устройства
                            result = self.sc200(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'megatec':
                            # Вызываем метод megatec передавая ему ip адрес устройства
                            result = self.megatec(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'apc':
                            # Вызываем метод apc передавая ему ip адрес устройства
                            result = self.apc(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'eltek':
                            # Вызываем метод eltek передавая ему ip адрес устройства
                            result = self.eltek(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'macc':
                            # Вызываем метод macc передавая ему ip адрес устройства
                            result = self.macc(ip)
                            # Добавляем результат в список snmp_trap
                            self.snmp_trap.append(result)
                        elif key == 'modbus':
                            # Вызываем метод modbus передавая ему ip адрес устройства
                            result = self.modbus(ip)
                            if type(result) is tuple:
                                #
                                self._parse_result_modbus(result)
                            else:
                                # Добавляем результат в список snmp_trap
                                self.snmp_trap.append(result)
                else:
                    pass
                self.sleep(self.interval_time)
    
    # Метод останавливает работу SNMP Manager-а
    def snmp_stop(self):
        try:
            self.manager.close()
        except AttributeError:
            pass

    def _parse_result_modbus(self, result):
        #
        for line in result:
            # Добавляем результат в список snmp_trap
            self.snmp_trap.append(line)

        

if __name__ == '__main__':
    snmp = ThreadSNMPAsk()
    print(snmp.forpost_2('10.19.178.1', flag = True, timeout = 5, next = True))
    #print(snmp.macc('10.196.50.195', flag=True))
    time.sleep(30)
    
    

